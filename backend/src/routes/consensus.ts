...
router.post('/prereview', async (req, res) => {
  try {
    ...
    const quickResult = PreReviewService.quickCheck(content);
    ...
    // Full pre-review
    const result = await PreReviewService.reviewSkill(content, metadata);
...
router.post('/evaluate', async (req, res) => {
  try {
    const report: TechnicalReport = req.body;

    // Validate required fields
    const validationErrors: string[] = [];

    if (!report.evaluatorId) validationErrors.push('evaluatorId is required');
    if (!report.skillId) validationErrors.push('skillId is required');
    if (!report.methodology?.type) validationErrors.push('methodology.type is required');
    if (!report.scores?.overall) validationErrors.push('scores.overall is required');
    if (!report.recommendation) validationErrors.push('recommendation is required');
...
router.post('/calculate', async (req, res) => {
  try {
    ...
    if (tier && Object.values(ConsensusTier).includes(tier)) {
      consensusService.setTier(tier);
    }

    // Calculate consensus
    const result = await consensusService.calculateConsensus(skillId, reports);
...
router.post('/quick', (req, res) => {
  try {
    ...
    const result = consensusService.quickConsensus(reports);
...
router.get('/config', (req, res) => {
  const config = consensusService.getConfig();
...
router.get('/tiers', (req, res) => {
  const tiers = Object.values(ConsensusTier).map(tier => ({
    tier,
    config: consensusService.getConfig(), // Would need to be dynamic
  }));

  return res.status(200).json({
    available: Object.values(ConsensusTier),
    current: consensusService.getConfig().tier,
  });
});
...
router.post('/demo', async (req, res) => {
  try {
    ...
    // Run pre-review
    const preReviewResult = await PreReviewService.reviewSkill(
    ...
    // Calculate consensus
    const consensusResult = await consensusService.calculateConsensus(
      'demo_skill_123',
      mockReports
    );
 ...
 */
function generateMockReports(scenario: string): TechnicalReport[] {
  ...
  const baseReport = {
    ...
    timestamp: now,
    methodology: {
      type: MethodologyType.MANUAL_REVIEW,
      ...
      return [
        {
          ...
          evaluatorTier: AuditorTier.TIER2,
          methodology: { ...baseReport.methodology, type: MethodologyType.STATIC_ANALYSIS },
          findings: {
            ...baseReport.findings,
            critical: [{
              id: 'finding_1',
        ...
        },
        {
          ...
          evaluatorTier: AuditorTier.TIER2,
          methodology: { ...baseReport.methodology, type: MethodologyType.DYNAMIC_TESTING },
          findings: {
            ...baseReport.findings,
            critical: [{
              id: 'finding_1',
        ...
        },
        {
          ...
          evaluatorTier: AuditorTier.TIER3,
          methodology: { ...baseReport.methodology, type: MethodologyType.FUZZING },
          findings: {
            ...baseReport.findings,
            critical: [{
              id: 'finding_1',
    ...
    case 'inconclusive':
      ...
      return [
        ...
        },
        {
          ...
          recommendation: EvaluationRecommendation.CONDITIONAL,
        },
        {
          ...baseReport,
          id: 'report_3',
          evaluatorId: 'auditor_3',
          evaluatorTier: AuditorTier.TIER3,
          methodology: { ...baseReport.methodology, type: MethodologyType.MANUAL_REVIEW },
      ...
      return [
        {
          ...
          id: 'report_1',
          evaluatorId: 'auditor_1',
          evaluatorTier: AuditorTier.TIER1,
          methodology: { ...baseReport.methodology, type: MethodologyType.STATIC_ANALYSIS },
          findings: {
            ...baseReport.findings,
            low: [{
        ...
        },
        {
          ...
          evaluatorId: 'auditor_3',
          evaluatorTier: AuditorTier.TIER2,
          methodology: { ...baseReport.methodology, type: MethodologyType.PENETRATION_TESTING },
