...
exports.PreReviewService = void 0;
// ============================================================
// INJECTION PATTERNS
// ============================================================
/**
...
class PreReviewService {
     ...
     */
    static async reviewSkill(skillContent, skillMetadata) {
        const checks = [];
        const blockers = [];
        const warnings = [];
     ...
     */
    static checkInjectionPatterns(content) {
        const checks = [];
     ...
     */
    static checkResourceDeclarations(content, metadata) {
        const checks = [];
        // Price sanity check
        ...
        if (metadata.declaredDependencies && metadata.declaredDependencies.length > 0) {
            const mentionedDeps = metadata.declaredDependencies.filter(dep => content.toLowerCase().includes(dep.toLowerCase()));
...
exports.PreReviewService = PreReviewService;
