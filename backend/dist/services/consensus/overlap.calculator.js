...
exports.interpretOverlap = interpretOverlap;
const types_1 = require("./types");
/**
 * Calculate overlap of critical findings across all reports
 */
function calculateOverlap(reports, minOverlapThreshold = 0.66) {
    if (reports.length === 0) {
    ...
    const findingsByEvaluator = new Map();
    for (const report of reports) {
        findingsByEvaluator.set(report.evaluatorId, extractCriticalFindings(report));
    }
    ...
    const { uniqueFindings, consensusFindings, outlierFindings } = deduplicateAndClassify(allFindings, reports.length);
    return {
        globalOverlap,
 ...
 */
function extractCriticalFindings(report) {
    return [
 ...
 */
function calculatePairwiseOverlap(eval1, eval2, findings1, findings2) {
 ...
 */
function normalizeFindingId(finding) {
 ...
 */
function deduplicateAndClassify(findings, evaluatorCount) {
    ...
    for (const [id, group] of grouped) {
        ...
        const representative = group.reduce((best, current) => current.description.length > best.description.length ? current : best);
...
function countByCategory(findings) {
    const counts = {
        [types_1.FindingCategory.INJECTION]: 0,
        [types_1.FindingCategory.JAILBREAK]: 0,
        [types_1.FindingCategory.DATA_LEAK]: 0,
        [types_1.FindingCategory.RESOURCE_ABUSE]: 0,
        [types_1.FindingCategory.PRIVACY]: 0,
        [types_1.FindingCategory.AUTHENTICATION]: 0,
        [types_1.FindingCategory.LOGIC_ERROR]: 0,
        [types_1.FindingCategory.OTHER]: 0,
    };
    for (const finding of findings) {
 ...
 */
function interpretOverlap(overlap) {
    if (overlap >= 0.90) {
